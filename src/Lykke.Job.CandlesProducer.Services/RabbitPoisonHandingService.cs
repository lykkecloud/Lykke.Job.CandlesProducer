...
namespace Lykke.Job.CandlesProducer.Services
{
    public class RabbitPoisonHandingService<T> : IRabbitPoisonHandingService<T>, IDisposable where T : class
    {
        ...
        private readonly IMessageDeserializer<T> _messageDeserializer = new JsonMessageDeserializer<T>();
        private readonly IRabbitMqSerializer<T> _messageSerializer = new JsonMessageSerializer<T>();
